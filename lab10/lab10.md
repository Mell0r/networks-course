...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Оно увеличивается на 1 с каждым запросом
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification меняются, TTL остаётся неизменным (246)
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   Найдите первое сообщение протокола ICMP с эхо-запросом, поступившее на ваш компьютер (с уже измененным размером пакета).

   ![alt text](image-5.png)
   -  Более чем за две минуты на мой компьютер не поступило ни одного сообщения ответа (все ping-запросы помечены no response found).
